...
void PolicyModel::trainBatches()
{
    ...
    training_input = torch::from_blob(&input_batches[0], {BATCH_SIZE, input_size}).to(model.device);
    ...
    std::cout << training_target << std::endl;
    ...
    std::cout << training_output << std::endl;
}
...
void PolicyModel::trainTest()
{
    //Train
    trainBatches();
    std::cout << "Loss: " << training_loss << std::endl;
...
void PolicyModel::forwardPolicyMoveSort(BitBoard& board, Move* moves_begin, Move* moves_end)
{
    ...
    {
        if (model.output_ptr[i] > 0.1)
        {
            std::cout << i << ": " << model.output_ptr << std::endl; 
        }
    }
